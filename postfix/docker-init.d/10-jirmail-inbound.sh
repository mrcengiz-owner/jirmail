...
_write_pgsql_cf /etc/postfix/pgsql-virtual-domains.cf \
  "SELECT 1 FROM core_maildomain d INNER JOIN core_mailaccount a ON a.domain_id = d.id AND a.is_active = true WHERE d.is_active = true $(_jir_sql_exclude_reserved_domains) AND d.name='%s' LIMIT 1"
